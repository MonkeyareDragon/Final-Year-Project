...
from fitnestx.meal.models import Food, Category, Meal
from fitnestx.meal.serializers import FoodSerializer

@pytest.mark.django_db
def test_food_list_filtered(api_client):
    # Create mock image files for meal_image, food_image, and category_image
    ...
    # Make a GET request to the food-list-filtered endpoint with authentication header
    url = reverse('api_v1:meal-user:users-meal:food-list-filtered', args=[meal.id])
    headers = {
    ...
    # Check if the response data matches the serialized food data
